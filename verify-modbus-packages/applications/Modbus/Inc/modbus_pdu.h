 ...
 */
#ifndef APPLICATIONS_MODBUS_MODBUS_PDU_H_
#define APPLICATIONS_MODBUS_MODBUS_PDU_H_
...
typedef struct{
    uint8_t fc;          //功能码
    uint16_t rd_addr;    //读取寄存器地址
    uint16_t rd_nb;      //读取数量
    uint16_t wr_addr;    //写寄存器地址
    uint16_t wr_nb;      //写入数量
    uint8_t dlen;        //数据长度
    uint8_t *pdata;      //数据指针
...
int modbus_pdu_make(uint8_t *buf, const mb_pdu_t *pdu, mb_pdu_type_t type);//生成pdu帧, 返回帧长度, 失败返回0
int modbus_pdu_parse(const uint8_t *buf, int len, mb_pdu_t *pdu, mb_pdu_type_t type);//解析pdu帧, 成功返回帧长度, 帧错误返回0, 功能码不支持返回-1
...
#endif /* APPLICATIONS_MODBUS_MODBUS_PDU_H_ */
