...
int modbus_port_read_hold(uint16_t addr, uint16_t *preg)//读保持寄存器, 返回 : 0-成功, -2-地址错误
{
    MB_ASSERT(preg != NULL);
...
//重新实现 modbus_port_slave.c 中的回调函数
int modbus_port_write_hold(uint16_t addr, uint16_t reg)//写保持寄存器, 返回 : 0-成功, -2-地址错误, -3-值非法, -4-设备故障
{
    if (addr < MB_REG_ADDR_BEGIN)
    {
        return(-2);
...
{
    mb_inst_t *hinst = modbus_create(MB_BACKEND_TYPE_RTU, &mb_bkd_prm);
    ...
    while(1)
    {
        modbus_slave_fsm(hinst);//循环调用从机状态机
    }
}

static int modbus_sample_rtu_slave_startup(void)
{
    rt_thread_t tid = rt_thread_create("mb-rtu-slave", modbus_sample_thread, NULL, 2048, 5, 20);
    RT_ASSERT(tid != NULL);
    rt_thread_startup(tid);
    return(0);
}
INIT_APP_EXPORT(modbus_sample_rtu_slave_startup);
